...
class GameLauncher:
    ...
    def launch_game(self, game_info):
        try:
            ...
            if game_info["package"]:
                module_path = f"{game_info['package']}.{game_info['module']}"
            else:
            ...
            # After game exits, reinitialize pygame display for launcher
            pygame.display.quit()
            pygame.display.init()
            pygame.font.init()
            self.screen = pygame.display.set_mode(WINDOW_SIZE)
            ...
            # Ensure display is reinitialized on error
            pygame.display.quit()
            pygame.display.init()
            pygame.font.init()
            self.screen = pygame.display.set_mode(WINDOW_SIZE)
    ...
    def run(self):
        while self.running:
            ...
            instructions = font.render("Press ESC in any game to return to this menu", True, WHITE)
            instructions_rect = instructions.get_rect(
                centerx=WINDOW_SIZE[0] // 2,
                bottom=WINDOW_SIZE[1] - 20
            )
